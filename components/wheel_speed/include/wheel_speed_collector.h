 ...
 * 职责：原始计数器(PCNT 16 位)差值 → 回绕/清零复位判定 → 窗口脉冲增量
 *       → 频率/RPM 数学链（调用 wheel_speed_math）→ 最小脉冲间隔去抖。
 * 本层不知道数据源来自 PCNT 还是模拟器，只与"新计数"打交道。
 */
...
/*
 * 推进一个采样窗口。new_counter 为该轮 PCNT（或模拟源）当前计数值。
